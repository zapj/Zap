<template>
   <el-card class="box-card">
        ...
        <el-row :gutter="20">
            <el-col :sm="24" :md="12" :lg="12" >
                ...
                <ul style="position: absolute;top: 8px;" class="fm-breadcrumb">
                  <li><el-link :underline="false" @click="changePath(0)">
            ...
            <el-col :sm="24" :md="12" :lg="12" >
              <el-button title="上一级"><el-icon><ArrowLeftBold /></el-icon></el-button>
            </el-col>
    ...
    <el-table :data="tableData" ref="filemgrRef" min-height="200" style="width: 100%" :height="tableHeight" v-loading="pageState.tbLoading"  :highlight-current-row="true">
            ...
            <template #default="{ row, $index }">
                <el-button link @click.prevent="openFile($index)">
                    <template #icon>
                        ...
                        <Icon icon="ic:round-insert-drive-file" width="28" height="28"/></span>
                        <span v-else >
                        <Icon icon="ic:baseline-folder" width="28" height="28"  :inline="true" /></span>
                    </template>
                    
                    {{ row.name }}
                    </el-button>
                    <template v-if="row.is_symlink===true">  <el-text class="mx-1 color-zinc-200" type="info" size="small">   >  {{ row.symlink_to }}</el-text></template>
        ...
        <el-table-column fixed="right" label="操作" width="120">
          <template #default="scope">
            <el-button  :icon="Edit"  circle class="mr-2" title="编辑" @click="editFile(scope.$index)" />   
            <el-dropdown >
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  ...
                  <el-dropdown-item @click.prevent="coypFilePath(scope.$index)">复制文件路径</el-dropdown-item>
      ...
      <template #footer>
        <el-pagination background layout="total,prev, pager, next" :total="pageState.total" :page-size="pageState.pagesize" v-model:current-page="pageState.currentpage" :default-current-page="1" @change="handlePaginration"  />
...
function updateBreadCrumb(path){
  ...
  if (pathArr.length > 0){
    for (let index = 0; index < pathArr.length; index++) {
      const copyPaths = pathArr.splice()
...
const coypFilePath = (index) => {
  copyText(globalStore.lastFilePath+'/'+tableData.value[index].name, undefined, (error, event) => {
          if (error) {
